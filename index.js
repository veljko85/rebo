...
var createScene = function () {
  ...
  function changePosAndScaleFence(valueToCount, activeFence) {
    ...
    if (fencesArr[activeFence].type == "rebo90180")
  ...
  // MAIN POST MESH //////////////////////////////////////////////////////////////////////////
  BABYLON.SceneLoader.ImportMeshAsync("", "mesh/", "mainPostRebo.glb").then(
    (result) => {
  ...
  var createRightFence = (posX, posZ, rotY, type) =>
    BABYLON.SceneLoader.ImportMeshAsync("", "mesh/", "reboCombo7.glb").then(
      (result) => {
        ...
        function toActiveFence() {
          ...
          if (fencesArr[activeFence].type == "rebo180180")
        ...
        fakeFence.position = new BABYLON.Vector3(
          getAbsPosX(result.meshes[2]) - 0.015,
          0.9,
        ...
        result.meshes[2].addChild(fakeFence);
        ...
        let rebo180180 = result.meshes[2];
        rebo180180Arr.push(rebo180180);
        rebo180180.isVisible = false;
        let rebo90180 = result.meshes[1];
        rebo90180Arr.push(rebo90180);
        rebo90180.isVisible = false;

        let allFencesThis = new Array(rebo180180, rebo90180);

        allFences.push(allFencesThis);

        if (type == "rebo180180") rebo180180.isVisible = true;

        if (type == "rebo90180") {
          rebo90180.isVisible = true;
          setSmallerFenceSize(
            rightPost,
            rightPostCap,
            newFenceForwardSigns,
  ...
  function handleTabActivnes() {
    if (!document.hidden) {
      createRightFence(0.99, 0, 0, "rebo180180");
  ...
  function changeFenceFunction(c, d, h) {
    ...
    if (d == "rebo180180") {
      rebo180180Arr[h].isVisible = true;
    }
    if (d == "rebo90180") {
      rebo90180Arr[h].isVisible = true;
    }
  ...
  changeFence[0].addEventListener("click", () => {
    changeFenceFunction(180, "rebo180180", activeFence);
  });
  changeFence[1].addEventListener("click", () => {
    changeFenceFunction(90, "rebo90180", activeFence);
  });
    ...
    if (fencesArr[activeFence].type == "rebo180180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(180, "rebo180180", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "rebo90180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(90, "rebo90180", i);
        }
      }
    }
  };
